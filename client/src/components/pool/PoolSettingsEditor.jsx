...
function SeasonField({ value, seasons, creation, onChange }) {
  return (
    <div className="scoring-card">
      <span className="scoring-section-title">Season</span>
      {creation ? (
        ...
        <p>{seasons?.find((s) => s.id === value)?.name ?? value}</p>
      )}
    </div>
  );
}
...
function PoolTypeField({ value, poolTypes, creation, onChange }) {
  return (
    <div className="scoring-card">
      <span className="scoring-section-title">Pool Type</span>
      {creation ? (
        <select className="form-select" value={value} onChange={(e) => onChange(e.target.value)}>
          {poolTypes?.map(({ value: v, label }) => <option key={v} value={v}>{label}</option>)}
        </select>
      ) : (
        <p>{poolTypes?.find((t) => t.value === value)?.label ?? value}</p>
      )}
    </div>
  );
}
...
export default function PoolSettingsEditor({ poolId, data, mode, onSave, onCancel, saveLabel }) {
  ...
  return (
    <div className="stack">
      <NameField value={values.name} onChange={field("name")} />
